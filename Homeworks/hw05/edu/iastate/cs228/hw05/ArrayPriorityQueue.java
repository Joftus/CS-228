...
public class ArrayPriorityQueue<T extends Comparable<? super T>> implements PriorityQueueInterface<T>
	...
	public void add(T newEntry)
		...
		while (inOrder == false)
			{
			inOrder = true;
			for (int index = 0; index < queue.length - 1 && queue[index] != null && queue[index + 1] != null; index++)
				{
				int compare = queue[index].compareTo(queue[index + 1]);
				if (compare > 0)
					{
					T temp = queue[index];
	...
	public String toString()
		...
		String result = "";
		for (int index = 0; index < queue.length && queue[index] != null; index++)
			{
			if (index == 0)
				{
				result = "[" + queue[0];
