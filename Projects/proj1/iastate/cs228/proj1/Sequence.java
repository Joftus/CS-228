...
public class Sequence
	...
	public boolean equals(Object obj)
		{
		char[] current = this.getSeq();
		char[] other = ((Sequence) obj).getSeq();
		int similar = 0;
		for (int index = 0; index < current.length; index++)
			{
			char lowerCase1 = Character.toLowerCase(current[index]);
			char lowerCase2 = Character.toLowerCase(other[index]);
			if (lowerCase1 == lowerCase2)
				{
				similar++;
				}
			}
		if (obj != null && this.getClass() == obj.getClass() && similar == current.length)
			{
			return true;
			}
		return false;
	...
	public boolean isValidLetter(char let)
		{
		if (Character.isUpperCase(let) == true || Character.isLowerCase(let) == true || let == 0)
			{
			return true;
			}
		return false;
