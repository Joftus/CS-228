...
public class GenomicDNASequence extends DNASequence
	...
	public GenomicDNASequence(char[] gdnaarr)
		{
		super(gdnaarr);
		for (int index = 0; index < gdnaarr.length; index++)
			{
			DNASequence newDNA = new DNASequence(gdnaarr);
			if (!newDNA.isValidLetter(gdnaarr[index]))
				{
		...
		for (int filler = 0; filler < iscoding.length; filler++)
			{
			iscoding[filler] = false;
			}
		}
	...
	public void markCoding(int first, int last)
		...
		try
			{
			Sequence newSeq = new Sequence(this.getSeq());
			if (first < last && first > 0 && newSeq.seqLength() > last)
				{
	...
	public char[] extractExons(int[] exonpos)
		...
		for (int index = 0; index < exonpos.length - 1; index++)
			{
			if (exonpos[index] > exonpos[index + 1] && iscoding[index] == false)
				{
				throw new IllegalArgumentException("Noncoding position is found");
				}
			}
		char[] seq = this.getSeq();
		char[] extractedExons = new char[seq.length];
		boolean anExon = false;
		int posInSeq = 0;
		int newIndex = 0;
		for (int index = 0; index < exonpos.length - 1; index++)
			{
			anExon = true;
			int end = exonpos[index + 1];
			posInSeq = exonpos[index];
			while (anExon == true)
				{
				if (posInSeq == end)
					{
				...
				extractedExons[newIndex] = seq[posInSeq];
				newIndex++;
				posInSeq++;
				}
			}
