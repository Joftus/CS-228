...
public class DFS
	 ...
	 * DiGraph.java) /* and an empty stack of type LinkedStack<V>. It colors each
	 * vertex green /* and sets the value of each vertex to null in the pred map
	 * (see lecture code). /* Then as long as there is a green vertex w left, the
	 * method calls visitDFS() /* on the vertex w along with other parameters. /* If
	 * visitDFS() returns false, then this method returns null. /* Otherwise, it
	 * returns the stack containing the list of all vertices in /* a topological
	 * order, which is produced by the visitDFS() method. /* If aGraph is null, then
	...
	public static <V> LinkedStack<V> depthFirstSearch(DiGraph<V> aGraph)
		...
		// System.out.println();
		// Iterate through each green vertex calling visitDFS()
		for (V w : aGraph.vertices())
			{
		...
		// else, visitDFS returns the stack containing the list of all vertices
		// in a topologicial order
		return topoOrder;
		}
	 ...
	 * given in lecture for an undirected graph. Here, you need to modify the
	 * algorithm to make it work for a directed graph. Note that the edge iterator
	 * citer should be changed from type Iterator<V> to type Iterator<Edge<V,
	 * Integer>>, and that citer.next().getVertex(), instead of citer.next(), gives
	 * the vertex w. If the vertex w is red, then the graph has a cycle. So the
	...
	protected static <V> boolean visitDFS(DiGraph<V> aGraph, V s, HashMap<V, String> color, HashMap<V, V> pred,
		...
		{
		// check if acyclic
		// if acyclic (has no cycles) generate a stack named topoOrder
		// else
		// return false;
		color.put(s, "red"); // first vertex is colored red
